...
class Run(Base):
    ...
    # NEW: per-run detector config (enables multi-detector + reproducibility)
    detector_name: Mapped[str] = mapped_column(String(64), nullable=False, default="yolo_onnx")
...
class Step(Base):
    __tablename__ = "steps"
    __table_args__ = (
        # Optional but recommended: one step name per run
        UniqueConstraint("run_id", "name", name="uq_steps_run_id_name"),
    )
